...
class TestGetRole:
    def test_get_role_by_id_returns_200(
        self, api_client_auth, role_detail_url, admin_role
    ):
        response = api_client_auth.get(role_detail_url(admin_role.pk))
        assert response.status_code == status.HTTP_200_OK

    def test_get_role_by_id_nonexistent_returns_404(
        self, api_client_auth, role_detail_url
    ):
        response = api_client_auth.get(role_detail_url(999))
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_get_list_roles_returns_200(self, api_client_auth, roles_url):
        response = api_client_auth.get(roles_url)
        assert response.status_code == status.HTTP_200_OK

    def test_list_roles_returns_list(
        self, api_client_auth, roles_url, admin_role, cashier_role
    ):
        response = api_client_auth.get(roles_url)
        assert len(response.data) == 2

    def test_list_roles_fields_present(self, api_client_auth, roles_url, admin_role):
        response = api_client_auth.get(roles_url)
...
class TestPostRole:
    def test_create_role_returns_201(
        self, api_client_auth, roles_url, payload_role_cashier
    ):
        response = api_client_auth.post(roles_url, payload_role_cashier)
        assert response.status_code == status.HTTP_201_CREATED

    def test_create_role_persisted(
        self, api_client_auth, roles_url, payload_role_cashier
    ):
    ...
    def test_create_role_duplicate_name_returns_400(
        self, api_client_auth, roles_url, admin_role, cashier_role, payload_role_cashier
    ):
        response = api_client_auth.post(roles_url, payload_role_cashier)
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_create_role_duplicate_name_case_insensitive_returns_400(
        self, api_client_auth, roles_url, cashier_role, payload_role_cashier
    ):
...
@pytest.mark.django_db
class TestPatchRole:
    def test_patch_role_updates_name(
        self, api_client_auth, role_detail_url, cashier_role
    ):
    ...
    def test_patch_role_updates_description(
        self, api_client_auth, role_detail_url, cashier_role
    ):
        response = api_client_auth.patch(
    ...
    def test_patch_role_persists_changes(
        self, api_client_auth, role_detail_url, admin_role
    ):
    ...
    def test_patch_role_nonexistent_returns_404(self, api_client_auth, role_detail_url):
    ...
    def test_patch_role_duplicate_name_returns_400(
        ...
        response = api_client_auth.patch(
            role_detail_url(sales_manager_role.pk),
            {"name": purchasing_manager_role.name},
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_patch_role_same_name_on_self_returns_200(
        self, api_client_auth, role_detail_url, sales_manager_role
    ):
        response = api_client_auth.patch(
    ...
    def test_patch_role_unauthenticated_returns_401(
        self, api_client, role_detail_url, purchasing_manager_role
    ):
...
@pytest.mark.django_db
class TestFiltersRole:
    def test_search_roles(self, api_client_auth, admin_role, roles_url):
        response = api_client_auth.get(f"{roles_url}?search={admin_role.name}")
